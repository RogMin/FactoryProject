...
const char* ssid = "Galaxy A52244D"; 
...
<style>
    ...
    .div-u, .div-l, .div-d, .div-r, .div-s {
        border: 1px solid gray;
        font-size: 1.5rem;
    }
    .div-u { grid-area: 1 / 2 / 2 / 3; padding:2rem 3rem; border-radius:100% 100% 0 0;}
    .div-l { grid-area: 2 / 1 / 3 / 2; padding:2.95rem 2.4rem; border-radius:100% 0 0 100%;}
    .div-d { grid-area: 3 / 2 / 4 / 3; padding:2rem 3rem; border-radius:0 0 100% 100%;}
    .div-r { grid-area: 2 / 3 / 3 / 4; padding:2.95rem 2.4rem; border-radius:0 100% 100% 0;}
    .div-s { grid-area: 2 / 2 / 3 / 3; border: 1px solid red; padding:3rem 2.3rem; }
...
<div class="parent">
    <div class="div1">
        ...
        <!-- Новые кнопки -->
        <div class="parent-j" style="margin-top: 1rem;">
            <button class="div-l" id="btn-shift-left">← Shift</button>
            <button class="div-r" id="btn-shift-right">Shift →</button>
            <button class="div-u" id="btn-turn-left">↺ Turn</button>
            <button class="div-d" id="btn-turn-right">Turn ↻</button>
        </div>
    </div>
...
function initButtons() {
  ...
  // Новые действия
  document.querySelector('#btn-shift-left').addEventListener('click', ()=>{ websocket.send(JSON.stringify({dir:5})) });
  document.querySelector('#btn-shift-right').addEventListener('click', ()=>{ websocket.send(JSON.stringify({dir:6})) });
  document.querySelector('#btn-turn-left').addEventListener('click', ()=>{ websocket.send(JSON.stringify({dir:7})) });
  document.querySelector('#btn-turn-right').addEventListener('click', ()=>{ websocket.send(JSON.stringify({dir:8})) });
...
function toggleBg(btn, state) {
    btn.style.background = state ? '#ff0000' : '#ffffff';
}
...
{
  switch(direction) {
    case 11: forward(motor1, motor2, speed); break;
    case 12:
    case 22:
    case 32:
    case 42:
      motor1.brake(); motor2.brake(); break;
    case 21: back(motor1, motor2, speed); break;
    case 31: motor1.drive(-255); motor2.drive(255); break;
    case 41: motor1.drive(255); motor2.drive(-255); break;
    ...
    case 6:  // Shift Right
      ...
      motor2.drive(-255);
      break;
    case 7:  // Turn in place left
      motor1.drive(255);
      motor2.brake();
      break;
    case 8:  // Turn in place right
      motor1.brake();
      motor2.drive(255);
      break;
  }
